...
import { ArrowLeft, Play, Pause, SkipBack, SkipForward, Volume2, Zap, CheckCircle2, X } from 'lucide-react';
...
export default function AlbumDetailClient({ albumTitle, initialAlbum }: AlbumDetailClientProps) {
  ...
  const handleBoostSuccess = (response: any) => {
    ...
    if (album && paymentRecipients) {
      setPaymentSuccess({
        amount: boostAmount,
        title: album.title,
        type: 'album',
        recipients: paymentRecipients.map((r: any) => ({
          name: r.name || album.artist || 'Artist',
          split: r.split || 0,
          address: r.address
        }))
      });
            ...
            <div className="relative bg-gradient-to-b from-gray-900 to-black rounded-2xl shadow-2xl w-full sm:max-w-md max-h-[85vh] sm:max-h-[90vh] overflow-hidden animate-in zoom-in-95 duration-300">
              ...
              <div className="p-6 space-y-4 overflow-y-auto max-h-[calc(85vh-8rem)] sm:max-h-[calc(90vh-10rem)]">
                ...
                <BitcoinConnectPayment
                  ...
                  onSuccess={(response) => {
                    // Handle track boost success with track-specific data
                    const trackRecipients = getTrackPaymentRecipients(selectedTrack);
                    if (album && trackRecipients) {
                      setPaymentSuccess({
                        amount: trackBoostAmount,
                        title: selectedTrack.title,
                        type: 'track',
                        recipients: trackRecipients.map((r: any) => ({
                          name: r.name || album.artist || 'Artist',
                          split: r.split || 0,
                          address: r.address
                        }))
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-gradient-to-br from-gray-800 to-gray-900 border border-green-500/30 rounded-2xl p-8 max-w-md w-full shadow-2xl relative">
            ...
            <div className="text-center mb-6">
              <h2 className="text-2xl font-bold text-white mb-2">Boost Sent! ⚡</h2>
            ...
            {/* Payment Details */}
            <div className="bg-black/30 rounded-lg p-4 mb-4 space-y-3">
              ...
              <div className="flex flex-col pt-2 border-t border-gray-700">
                <span className="text-gray-400 text-sm mb-1">
                  {paymentSuccess.type === 'album' ? 'Album' : 'Track'}
                </span>
            ...
            {/* Payment Splits */}
            {paymentSuccess.recipients.length > 0 && (
              <div className="bg-black/30 rounded-lg p-4 mb-6">
                <p className="text-gray-400 text-sm font-semibold mb-3">Payment Split:</p>
                <div className="space-y-2 max-h-40 overflow-y-auto">
                  {paymentSuccess.recipients.map((recipient, idx) => {
                    const totalSplit = paymentSuccess.recipients.reduce((sum, r) => sum + r.split, 0);
                    const percentage = totalSplit > 0 ? ((recipient.split / totalSplit) * 100).toFixed(1) : '0';
                    const amount = totalSplit > 0 ? Math.floor((paymentSuccess.amount * recipient.split) / totalSplit) : 0;
                    return (
                      <div key={idx} className="flex items-center justify-between text-sm">
                        <div className="flex items-center gap-2 flex-1 min-w-0">
                          <div className="w-2 h-2 rounded-full bg-green-500 flex-shrink-0" />
                          <span className="text-gray-300 truncate">{recipient.name}</span>
                        </div>
                        <div className="flex items-center gap-3 flex-shrink-0">
                          <span className="text-gray-400">{percentage}%</span>
                          <span className="text-green-400 font-medium">{amount} sats</span>
                        </div>
                      </div>
                    );
                  })}
