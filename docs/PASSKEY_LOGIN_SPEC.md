...
> Source: [breez/passkey-login](https://github.com/breez/passkey-login/blob/main/spec.md)
...
No seed phrase, backup, server-side state, or cloud storage is required. Any device that can authenticate via passkey+PRF can reconstruct root keys.

From each **root key** (derived per salt), **BIP39/BIP32 is applied** to create the mnemonics and deterministic key hierarchies that can be used in wallets and compatible apps. Optionally, users can **write down the mnemonic phrase** derived from a root key for backup/recovery in case the passkey is lost or removed.
