...
async function parseDoerfelverseFeed() {
  console.log('🔄 Fetching Into the Doerfelverse feed...');
  
  try {
    // Fetch the feed
    const feedContent = await fetchFeed('https://www.doerfelverse.com/feeds/intothedoerfelverse.xml');
    ...
    console.log(`📻 Found ${items.length} episodes in Into the Doerfelverse feed`);
    ...
    // Find and update intothedoerfelverse feed
    const feedIndex = currentData.feeds.findIndex(f => f.id === 'intothedoerfelverse');
    ...
    if (feedIndex !== -1) {
      ...
      console.log('✅ Successfully updated Into the Doerfelverse feed!');
      console.log(`📊 Total episodes: ${tracks.length}`);
      console.log(`🆕 Latest: ${tracks[0].title}`);
    } else {
      console.error('❌ intothedoerfelverse feed not found in parsed-feeds.json');
    }
