...
async function extractColorsFromImage(imageUrl, albumTitle) {
  try {
    console.log(`🎨 Extracting colors for: ${albumTitle}`);
    ...
    if (pixels.length === 0) {
      console.log(`⚠️  No valid pixels found for ${albumTitle}, using fallback`);
      return {
    ...
    console.log(`✅ Extracted for ${albumTitle}:`, result.dominant);
    return result;
    
  } catch (error) {
    console.error(`❌ Failed to extract colors for ${albumTitle}:`, error.message);
    return {
      dominant: 'rgb(64, 64, 64)',
      palette: generateColorPalette(64, 64, 64),
      isDark: true
...
async function extractAllColors() {
  try {
    ...
    console.log(`🚀 Starting color extraction for ${data.albums.length} albums...`);
    ...
    for (let i = 0; i < data.albums.length; i++) {
      ...
      if (!album.coverArt) {
        console.log(`⏭️  Skipping ${album.title} - no cover art`);
        continue;
      }
      
      // Extract colors
      const colors = await extractColorsFromImage(album.coverArt, album.title);
      
      // Update album with extracted colors
      data.albums[i].colors = colors;
    ...
    console.log(`🎉 Successfully extracted colors for all albums!`);
...
module.exports = { extractAllColors };